...
const SAPExperts = () => {
  return (
    <div className="bg-gradient-to-r from-blue-300 to-blue-800 text-white md:px-[160px] md:py-[80px]">
      <h2 className="text-4xl font-bold text-center mb-4">Meet Our Elite SAP Experts</h2>
    ...
    image: string;
          ...
          <div key={index} className="bg-white text-black rounded-2xl overflow-hidden shadow-lg flex flex-col">
            ...
            <div className="p-6">
              ...
              <div className="w-full bg-gray-200 h-2 rounded-full my-2">
                <div className="bg-green-500 h-2 rounded-full" style={{ width: `${expert.match}%` }}></div>
              </div>
              <p className="text-sm mt-2"><strong>Persona:</strong> {expert.persona}</p>
              <p className="text-sm"><strong>Availability:</strong> {expert.availability}</p>
              <p className="text-sm"><strong>Domain Fit:</strong> {expert.domainFit}</p>
              <p className="text-sm"><strong>Reference Score:</strong> {expert.referenceScore}</p>
              <p className="text-sm"><strong>Readiness:</strong> {expert.readiness}</p>
              <button className="mt-4 w-full py-2 bg-gray-900 text-white rounded-lg">View Full Persona Report →</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="text-center mt-12">
        <button className="bg-yellow-400 text-black px-6 py-3 rounded-full hover:bg-yellow-300 transition font-bold">View More SAP Consultants →</button>
      </div>
    </div>
  )
}
